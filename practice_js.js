...
//console.log(signupform.nameadd.value);
	...
	/*	sessionStorage.removeItem("count_code");
		sessionStorage.removeItem("language");*/
		var count_code=Number(sessionStorage.getItem("count_code"));
		var lang=sessionStorage.getItem("language");
		if (lang==null||lang=="python") {lang="python";}
		else{
			$("#python").removeClass("active1");
			$("#"+lang).addClass("active1");
		}
		var file="json_code\\"+lang+"\\json_ex"+String(count_code)+".json";
		if (count_code==0) {
			file="json_code\\"+lang+"\\json_ex1.json";
			loadfile(file);
		}
		if (count_code>0) {
			loadfile(file);
		...
		$("#restart").click(function(){
			sessionStorage.setItem("count_code",count_code);
			sessionStorage.setItem("language",lang);
			location.reload();
		});
		$("#next_sample").click(function(){
			if (count_code==0) { count_code=1;}
			let temp=count_code+1;
			sessionStorage.setItem("count_code",temp);
			sessionStorage.setItem("language",lang);
			location.reload();
		});
		$('#feed').click(function(){
...
loadfile=function(file){
		...
		.then(function(json){
			...
			sessionStorage.setItem("count_code",0);
			location.reload();
		});
function call(json){
	...
	$('#keyboard li').click(function(){
		...
		// Special characters
		else if ($this.hasClass('symbol')) character = $('span:visible', $this).text();
